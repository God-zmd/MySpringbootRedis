...
@WebServlet("/redisAddServlet")
public class RedisAddtServlet extends HttpServlet {
    @Resource
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        //实例化一个stunent对象，将要添加的值存进对象
        Student stu=new Student();
        stu.setStuId(stuId);
        stu.setStuName(stuName);
        Date data=new SimpleDateFormat("yyyy-MM-dd").parse(birthday);//将日期字符串转换位date
        Timestamp birth=new Timestamp(data.getTime());
        stu.setBirthday(birth);//将date转换位timstamp
        ...
        //将新添加的数据存到hash
       redisTemplate.opsForHash().put("stuHashMap",uuid+"",stu);
       //将uuid作为属性加入sortedSet
        redisTemplate.opsForZSet().add("avgScoreZSet",uuid+"",stu.getAvgscore());
